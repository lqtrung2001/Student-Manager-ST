...
export const login = async (req, res) => {
  ...
  //   Users.findOneAndUpdate({username: username}, {password: '$argon2i$v=19$m=4096,t=3,p=1$MnlmZGprNDNuZGtmbjNuZGZrbjNybm9uM24zbjIx$+vZBGdp14j/iixURKFhlYA'})
  //   console.log('doi thanh cong')

  // }catch(error){
  //   console.log('doi sai roi')
  // }
  try {
    const user = await Users.findOne({ username: username });
    console.log(user)
    const student = await Student.findOne({ msv: username });
    // console.log('studfent', student)
